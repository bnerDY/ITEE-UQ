...
#else
	/* NON AVR
	 * We turn off echo, ignore carriage returns and disable
	 * canonical mode so characters can be processed one at a time
	 * - not a whole line at a time.
	 * We also make stdin and stdout non buffered.
	 */
	struct termios tty;
	tcgetattr(0, &tty);
	tty.c_iflag &= ~(IGNCR);
...
#else
	/* Non AVR - check if any bytes can be read from stdin (fd 0) */
