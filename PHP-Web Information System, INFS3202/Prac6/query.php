<?php
	...
	if(isset($_POST['func']) && isset($_POST['id']) && $_POST['func']=="addmap"){
	...
	if(isset($_POST['input'])){
		if(count(explode(" ",$_POST['input']))>1) {
			...
			if (strpos($input2, '<') !== false) {
				...
				if($result) {
					while($row = mysql_fetch_array($result)){
						echo $row['category']."  ".$row['name']."  ".$row['price']."  ".$row['dueTime']."  ".$row['location']."  ".$row['description']."  ".$row['imageLink']."  ".$row['review']."  ".$row['Latitude']."  ".$row['Longitude']."@";
					}				
				}
			} 
		}
